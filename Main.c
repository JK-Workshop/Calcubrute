...
main(int p_numArgs, char** p_args)
{
    ...
    ccbMemoryInit(pContext, pMemory, 1ull << 20);
    ...
    //     base[i] = i;
    // }
    const VkDeviceAddressRangeKHR range = {pMemory->deviceLocalDeviceBase, 9 * sizeof(uint32_t)};
    vkCmdFillMemoryKHR(cmdBuf, &range, VK_ADDRESS_COMMAND_FULLY_BOUND_BIT_KHR, 114514u);
    const VkMemoryRangeBarrierKHR barrier = {
    ...
    const VkDependencyInfo dep = {VK_STRUCTURE_TYPE_DEPENDENCY_INFO, &barrier};
